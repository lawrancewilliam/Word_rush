<script>
  ...
  async function checkSession() {
    ...
    try {
      const { data: existing, error: fetchError } = await supabase
        .from('players')
        .select('*, games!inner(id, status, game_number, projector_mode)')
      ...
      if (fetchError) {
        if (fetchError.code === 'PGRST205' || fetchError.message?.includes('Could not find the table')) {
          error = 'Game database not configured. Please ask the host to set up the game.';
          loading = false;
          screen = 'ERROR';
          return;
        }
      ...
      if (existing) {
        ...
        currentGame = existing.games;

        if (currentGame.status === 'LOBBY_OPEN') {
          screen = 'LOBBY';
          await subscribeToGame(currentGame.id);
          await loadPlayers(currentGame.id);
        } else if (currentGame.status === 'COUNTDOWN') {
          screen = 'COUNTDOWN';
          await subscribeToGame(currentGame.id);
        } else if (currentGame.status === 'PLAYING' || currentGame.status === 'QUESTION_LOCKED') {
          screen = 'PLAYING';
          await subscribeToGame(currentGame.id);
        ...
        if (lobbyError) {
          if (lobbyError.code === 'PGRST205' || lobbyError.message?.includes('Could not find the table')) {
            error = 'Game database not configured. Please ask the host to set up the game.';
            loading = false;
            screen = 'ERROR';
            return;
  ...
  async function showWaitingState() {
    try {
      ...
      if (!allGames || allGames.length === 0) {
        screen = 'WAITING';
        error = 'Waiting for the next game to begin.';
        return;
      }

      const allCompleted = allGames.every(g => g.status === 'COMPLETED');
      const anyPlaying = allGames.some(g => ['PLAYING', 'QUESTION_LOCKED', 'COUNTDOWN'].includes(g.status));
      const anyLobbyClosed = allGames.some(g => g.status === 'LOBBY_CLOSED');

      if (allCompleted) {
        screen = 'ALL_COMPLETED';
      } else if (anyPlaying) {
        screen = 'GAME_PLAYING';
        error = 'Game in progress. Please wait for the next game.';
      } else if (anyLobbyClosed) {
        screen = 'WAITING';
        error = 'Game starting soon. Please wait...';
      } else {
        screen = 'WAITING';
        error = 'Waiting for the next game to begin.';
      }
  ...
  async function joinGame() {
    ...
    try {
      const { data, error: rpcError } = await supabase.rpc('join_active_game', {
...
{#if screen === 'CONFIG_ERROR' || !supabaseReady}
    ...
    <div class="w-full max-w-md animate-fade-in text-center space-y-6">
      ...
      <div class="glass-strong rounded-2xl p-6 space-y-4">
        <div class="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-700">
          ...
          <p class="text-amber-600">Supabase credentials are not configured.</p>
        </div>
        <p class="text-gray-400 text-xs">Ask the host to set up the game.</p>
      </div>
    </div>
  </div>
    ...
    <div class="w-full max-w-md animate-scale-in">
      <div class="text-center mb-6">
        <div class="inline-block px-4 py-1.5 rounded-full bg-purple-100 border border-purple-200 text-purple-700 text-xs font-semibold tracking-wider mb-4">
          {currentGame?.game_number ? `GAME ${currentGame.game_number}` : 'GAME'}
        </div>
    ...
    <div class="w-full max-w-md animate-scale-in text-center space-y-6">
      ...
      <div class="glass-strong rounded-2xl p-8 space-y-4">
        ...
        <h2 class="text-2xl font-bold text-gray-900">
          Game {currentGame?.game_number || ''} Completed!
        </h2>
  ...
  <div class="min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md animate-scale-in text-center space-y-6">
      ...
      <div class="glass-strong rounded-2xl p-8 space-y-4">
        ...
        <p class="text-gray-500">Thanks for participating!</p>
      </div>
    </div>
  </div>
  ...
  <div class="min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md animate-scale-in text-center space-y-6">
      ...
      <div class="glass-strong rounded-2xl p-8 space-y-4">
        ...
        <p class="text-gray-500">{error || 'The host will open the lobby soon.'}</p>
      </div>
    </div>
  </div>
